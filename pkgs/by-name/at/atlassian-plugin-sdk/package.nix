...
stdenv.mkDerivation rec {
  ...
  src = fetchurl {
    url = "${mavenGroupIdUrl}/${pname}/${version}/${pname}-${version}.tar.gz";
  ...
  passthru = {
    ...
    updateScript = writeShellScript "update-${pname}" ''
      ...
      NEW_VERSION=$(curl -s ${mavenGroupIdUrl}/${pname}/maven-metadata.xml | xq -r '.metadata.versioning.latest')
      ...
      DOWNLOAD_URL="${mavenGroupIdUrl}/${pname}/${version}/${pname}-$NEW_VERSION.tar.gz"
      NIX_HASH=$(nix hash to-sri sha256:$(nix-prefetch-url $DOWNLOAD_URL))

      update-source-version "${pname}" "$NEW_VERSION" "$NIX_HASH" "$DOWNLOAD_URL"
    '';
  };
